...
def healthcheck():
    response = app.response_class(
        response=json.dumps({"result": "OK - I hear you now"}),
        status = 200,
...
def metrics():
    ...
    connection.close()
    response = app.response_class(
        response=json.dumps({"post_count": post_count[0], "db_connection_count": app.config['DB_CONNECTION_COUNTER']}),
        status = 200,
        mimetype='application/json'
    )
...
def post(post_id):
    ...
    if post is None:
      app.logger.info(f'a non existing article with id: {post_id} is accessed')
...
if __name__ == "__main__":
   ...
   stderror_handler = logging.StreamHandler(sys.stdout)
